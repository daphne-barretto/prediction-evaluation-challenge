"""prong_ab_logit — logit-space ensemble of sub 33 (kNN) and sub 32 (Ridge).

Same component models as prong_ab_ensemble (sub 45), but the final blend
happens in logit space instead of probability space:

    logit(p_final) = W_KNN * logit(p_knn_final) + (1 - W_KNN) * logit(p_ridge)

Sub 45 (probability-space blend) tied at -0.61 / 0.69, dropping 0.013 NLL
from sub 33. Logit-space blending may preserve sub 33's calibration better.
"""
...
def predict(input: dict, labeled: list[dict] | None = None) -> float:
    ...
    z_final = W_KNN * _logit(p_knn_final) + (1.0 - W_KNN) * _logit(p_ridge)
