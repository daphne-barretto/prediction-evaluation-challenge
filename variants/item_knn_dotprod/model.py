"""item_knn_dotprod — Sub 33 without per-query L2 normalization (raw dot product).
...
def _encode_item(text: str) -> np.ndarray:
    ...
    proj = raw @ _PCA_COMPONENTS_T  # (256,) — no L2 normalization (dot product variant)
    _item_emb_cache[cache_key] = proj.astype(np.float32)
    return _item_emb_cache[cache_key]
