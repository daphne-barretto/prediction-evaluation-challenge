"""item_knn_logit_b04 — sub 33 blend done in logit space, not probability space.

Replaces the probability-space blend
    p = BETA * p_knn + (1 - BETA) * p_subj
with the logit-space blend
    logit(p) = BETA * logit(p_knn) + (1 - BETA) * logit(p_subj)

Same hyperparameters (K=20, TEMP=0.05, BETA=0.4, ALPHA=0.1) so this is a
clean apples-to-apples blend-space ablation versus sub 33.
"""
...
def predict(input: dict, labeled: list[dict] | None = None) -> float:
    ...
    if p_knn is None:
        ...
        z = BETA * _logit(p_knn) + (1.0 - BETA) * _logit(p_subj)
