"""sub13_plus_shift — Sub 13 (winning blend) + shrunk global K=5 shift.

Sub 13:        p_blend = 0.5 * p_sub5 + 0.5 * subject_mean_acc  (NLL = -0.61)
Sub 14:        p_sub5  - shrunk_global_K_shift                    (NLL = -0.65, no-op)

Sub 14 was a no-op on top of sub 5 because T-scaled mpnet outputs sit
near 0.5, where the mean residual `mean(p - y)` is dominated by noise.
Sub 13's predictions sit near (0.5 + 0.645)/2 ≈ 0.57 instead, where the
residual signal has a defined mean. This variant stacks sub 14's logic
(shrunk, capped global shift from labeled examples) onto sub 13's blend
to test whether the K=5 channel adds value when applied to a non-flat
prediction distribution.

Recipe:
    p_blend = 0.5 * p_sub5 + 0.5 * subject_mean_acc
    raw     = mean(p_blend(x_i) - y_i)  over K labeled examples
    shrunk  = raw * n / (n + LAMBDA)
    shift   = clip(shrunk, -CAP, +CAP)
    p_final = clip(p_blend - shift, eps, 1 - eps)

If this beats sub 13 (-0.61), the K=5 channel is finally contributing
positive signal; if it ties, the labels are still noise-dominated;
if it regresses, sub 13 is already pulled to the right calibration
manifold and further shifts move it off.

Ships NO labeling.py so the platform supplies random labels (sub 14's
rationale: random gives an unbiased mean-residual estimator; the
uncertainty×diversity acquisition would bias the sample toward hard
cases).
"""
...
BLEND_WEIGHT_SUB5 = 0.5  # matches sub 13
LAMBDA = 25.0            # matches sub 14
SHIFT_CAP = 0.05
...
def _blend_predict(ex: dict) -> float:
    ...
    p_subj = _lookup_subject_mean_acc(ex["subject_content"])
    return BLEND_WEIGHT_SUB5 * p_sub5 + (1.0 - BLEND_WEIGHT_SUB5) * p_subj
