"""sub33_label_platt — sub 33 + Platt scaling (a, b) fit on labeled[].

Extends sub 33 with a 2-parameter logit-space recalibration:

    z_final = a * z_base + b
    p_final = sigmoid(z_final)

where z_base = logit(p_sub33(input)) and (a, b) are fit from the K=25
labeled[] examples by minimizing logistic loss with a strong ridge prior
pulling (a, b) toward the identity transform (a=1, b=0):

    L(a, b) = sum_i BCE(sigmoid(a*z_i + b), y_i)
            + lambda_a * (a - 1)^2 + lambda_b * b^2

Fit by 5 Newton-Raphson steps on the unconstrained (a, b) (a clamped to
[0.5, 2.0] after each step to avoid pathological inversions). With
n=25 labels and lambda_a=lambda_b=10, the prior dominates noise so
in expectation (a, b) -> (1, 0) and the model collapses to sub 33.

Compared to sub33_label_shift (1 dof, intercept-only), Platt has 2 dof
and can correct mild slope-miscalibration in sub 33's logits.
