"""sub5_blend_global — prior-pull control for sub 13.

p_blend = 0.5 * p_sub5 + 0.5 * GLOBAL_MEAN_ACC  (≈ 0.645)

Diagnostic for sub 13's win. Sub 13 (NLL=-0.61) blends with the
per-subject mean; this variant blends with the *global* mean (a single
scalar shared across all subjects). If this scores essentially the same
as sub 13, the lever is just "pull predictions toward the training-time
prior"---per-subject identity contributes nothing. If this scores
noticeably worse (closer to -0.65), the per-subject signal is doing
real work.

Note: T-scaled sub 5 already sits near 0.5, so blending with 0.645 just
nudges every prediction toward 0.57; arithmetic blend, no per-subject
variation.
"""
...
def predict(input: dict, labeled: list[dict] | None = None) -> float:
    ...
    p_subj = _GLOBAL_MEAN_ACC  # prior-pull control: blend with global mean instead of per-subject
