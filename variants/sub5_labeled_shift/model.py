"""sub5_labeled_shift — Sub 5 + a *single shrunk global* offset from K=5 labels.

Slide 34 (adaptive labels): "use for light per-run calibration; not training".
Sub 8 attempted per-subject mean-residual offsets and lost 0.10 NLL because
25 labels spread over ~5 subjects gives ~5 per-subject and the resulting
offsets are dominated by noise. This variant tries the strictly more
conservative play: estimate one global offset from all available labels and
apply it everywhere, with shrinkage proportional to the sample size and a
hard cap on magnitude to keep a single outlier from blowing up NLL.

Concretely, for residuals r_i = p_sub5(x_i) - y_i over the K labeled items:

    raw_shift = mean(r_i)
    shrunk    = raw_shift * n / (n + LAMBDA)
    shift     = clip(shrunk, -CAP, +CAP)
    p_final   = clip(p_sub5 - shift, eps, 1 - eps)

With n=25, LAMBDA=25, CAP=0.05 the worst-case nudge is ±5 pp; with the
expected std of mean residuals around 0.02 after shrinkage, the typical
adjustment is much smaller. This is the smallest piece of the "use the
labels" lever that has a clean theoretical justification (correcting global
calibration drift between train and the leaderboard sample mean).

This variant intentionally ships NO labeling.py so the platform supplies
*random* labels (per slide 34: "fallback if missing... random sample").
Random labels give an unbiased estimator of mean residual; the
uncertainty×diversity labeling.py we use elsewhere biases sampling toward
hard cases and would skew the mean-residual estimate.
"""
...
LAMBDA = 25.0   # shrinkage strength; with n=25 labels this halves the raw shift
SHIFT_CAP = 0.05  # never adjust by more than 5 percentage points
...
def _sub5_predict(ex: dict) -> float:
    theta = _lookup_theta(ex["subject_content"])
    x = _build_x(theta, ex["subject_content"], ex["item_content"],
                 ex["benchmark"], ex["condition"])
    return _platt_prob(_logit(x))


_shift_cache: dict[int, float] = {}


def _compute_global_shift(labeled: Iterable[dict]) -> float:
    """Mean(pred - actual) on labeled, shrunk and capped."""
    residuals: list[float] = []
    for ex in labeled:
        try:
            p = _sub5_predict(ex)
            y = float(ex["label"])
            residuals.append(p - y)
        except Exception:
            continue
    if not residuals:
        return 0.0
    n = len(residuals)
    raw = float(np.mean(residuals))
    shrunk = raw * n / (n + LAMBDA)
    return float(np.clip(shrunk, -SHIFT_CAP, SHIFT_CAP))


def _get_global_shift(labeled: list[dict] | None) -> float:
    if not labeled:
        return 0.0
    key = id(labeled)
    if key not in _shift_cache:
        _shift_cache[key] = _compute_global_shift(labeled)
    return _shift_cache[key]


def predict(input: dict, labeled: list[dict] | None = None) -> float:
    p = _sub5_predict(input)
    shift = _get_global_shift(labeled)
    p_adj = p - shift
    return float(np.clip(p_adj, 1e-4, 1 - 1e-4))
