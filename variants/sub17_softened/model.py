"""sub17_softened — sub 17 (alpha=0.3 blend) with an extra 5% pull toward
the global mean. Targets a 3rd-decimal NLL improvement while leaving
ranking (AUC) essentially unchanged.

Sub 17 predictions are likely slightly over-confident for hard subjects;
shrinking toward the global mean is the maximum-likelihood prior for
unknown samples and acts as a soft cap on log-loss penalties.

p_soft = 0.95 * (0.3 * p_sub5 + 0.7 * subject_mean_acc) + 0.05 * global_mean

Identical artifacts as sub 17.
"""
...
def predict(input: dict, labeled: list[dict] | None = None) -> float:
    ...
    p_soft = (1.0 - SOFTEN_WEIGHT) * p_blend + SOFTEN_WEIGHT * _GLOBAL_MEAN_ACC
    return float(np.clip(p_soft, 1e-4, 1 - 1e-4))
