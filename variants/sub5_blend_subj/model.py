"""sub5_blend_subj — Sub 5 (T-scaled mpnet) blended with subject mean.

Slide 31 (option-space map) classifies metadata signals as a "stabilizer
for sparse rows": when the content-based model is noisy at the per-item
level but the *per-subject* mean is a well-calibrated prior on overall
accuracy, an equal-weight blend can reduce variance with a small ranking
loss. Concretely:

    p_blend = 0.5 * p_sub5 + 0.5 * subject_mean_acc

Both inputs live in the same probability space; the arithmetic mean is the
plain average and is robust without needing a tuned weight (we'd need a
held-out calibration set to fit one, and the cold-start val set is the
same one sub5's T was fit on, so we don't fit further to avoid leaking).

Compared to sub 8 (sub 5 + per-subject mean-residual offset on the K=5
labels, NLL=-0.75 — worse), the residual signal there came from labeled
items; this one uses a strictly larger, label-free signal
(`subject_mean_acc` is computed on all of training). It should not regress
the way sub 8 did.

Falls back to global mean when the subject is unseen.
"""
...
BLEND_WEIGHT_SUB5 = 0.5  # arithmetic mean of sub-5 prob and subject mean
